...
Wrap.prototype.startServer=function(propOverrides, done) {
  ...
  function servPathExists(cb)
  {
    fs.stat(self.MC_SERVER_PATH,function(err,stat){
      if(err)
        return cb(err);
      if(!stat.isDirectory())
        return cb(new Error("Can't create server path"));
      cb();
    });
  }
  ...
  batch.push(function(cb) {
    servPathExists(function(err){
      if(err)
        return fs.mkdir(self.MC_SERVER_PATH, cb);
      cb();
    })
  });
  batch.push(servPathExists);
  batch.push(function(cb) {
    var str = "";
