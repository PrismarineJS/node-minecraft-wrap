...
{
  constructor(clientPath,version,maxMem=1024,doneRegex=new RegExp(/\[.+\]: Narrator library successfully loaded/)) {
    super();
  ...
  auth(username,password) {
    ...
    const doAuth=(auths) => {
      ...
      let k;
      if(!username && auths['selectedUser'])
        k=auths['selectedUser'];
      else {
      ...
      const initialAuth=() => {
        debug('initial auth',auths['clientToken'].replace(/-/g,''),username,password);
        return auth({
          ...
          .then((data) => {
            const {accessToken,clientToken,selectedProfile:{id:wuuid,name:displayName},user:{id:userid}}=data;
            ...
            this.setAuthInfo(session['displayName'],wuuid,session['accessToken']);
      ...
      if(k===null) {
        ...
        return validate(session['accessToken'])
          .then(() => {
            this.setAuthInfo(session['displayName'],k,session['accessToken']);
            return fs.writeFile(this.clientPath+"/launcher_profiles.json",JSON.stringify(auths,null,2));
          })
          .catch(() => {
    ...
    return fs.readFile(this.clientPath+"/launcher_profiles.json","utf8")
      ...
      .catch(() => ({authenticationDatabase:{}}))
      .then(auths => doAuth(auths));
  }

  setAuthInfo(playerName,uuid,accessToken) {
    this.auth_player_name=playerName;
    this.auth_uuid=uuid;
    this.auth_access_token=accessToken;
    debug('auth info set to',this.auth_player_name,this.auth_uuid,this.auth_access_token);
  }
  ...
  start() {
   return new Promise((resolve) => {
     ...
     eval('`'+this.minecraftArguments+'`').split(" ").forEach(arg => args.push(arg));
     debug(args);
