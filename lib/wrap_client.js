...
class WrapClient extends EventEmitter {
  constructor (clientPath, version, os = 'linux', maxMem = 1024, customMainClass, customMinecraftArguments, customLibraries, doneRegex = new RegExp(/\[.+\]: Narrator library successfully loaded/)) {
    super()
    this.clientPath = clientPath || mcDefaultFolderPath
    this.version = version
    this.os = os
    this.maxMem = maxMem
    this.customMainClass = customMainClass || null
    this.customMinecraftArguments = customMinecraftArguments || null
    this.customLibraries = customLibraries || null
    this.doneRegex = doneRegex
    this.launcher = new LauncherDownload(this.clientPath, this.os)
  }

  prepare () {
    return this.launcher.getWholeClient(this.version)
      .then(({client, libraries, nativesPath}) => {
  ...
  start () {
    return new Promise((resolve) => {
      ...
      let allLibrariesJar = this.libraries.join(this.os.toLocaleLowerCase() === 'windows' ? ';' : ':')
      if (this.customLibraries != null) {
        allLibrariesJar = this.customLibraries.join(this.os.toLocaleLowerCase() === 'windows' ? ';' : ':')
      }
      const minecraftJar = this.minecraftJar
      let mainClass = this.mainClass
      if (this.customMainClass != null) {
        mainClass = this.customMainClass
      }
      ...
      const args = [
        ...
        '-cp',
        [allLibrariesJar, minecraftJar].join(this.os.toLocaleLowerCase() === 'windows' ? ';' : ':'),
        mainClass
      ]
      if (this.customMinecraftArguments != null) {
        eval('`' + this.customMinecraftArguments + '`').split(' ').forEach(arg => args.push(arg)) // eslint-disable-line no-eval
        debug(args)
        debug(this.customMinecraftArguments)
      } else {
