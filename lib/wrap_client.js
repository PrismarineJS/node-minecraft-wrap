...
class WrapClient extends EventEmitter {
  constructor (clientPath, version, os = 'linux', maxMem = 1024, doneRegex = new RegExp(/\[.+\]: Narrator library successfully loaded/)) {
    super()
  ...
  setCustom (minecraftArguments, mainClass, libraries) {
  ...
  start () {
    return new Promise((resolve) => {
      ...
      const allLibrariesJar = this.libraries.join(process.platform === 'win32' ? ';' : ':')
      ...
      const args = [
        ...
        '-cp',
        [allLibrariesJar, minecraftJar].join(process.platform === 'win32' ? ';' : ':'),
        mainClass
      ]
