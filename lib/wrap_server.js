  ...
  startServer (propOverrides, done) {
    ...
    mkdirp(this.MC_SERVER_PATH)
      ...
      .then(() => new Promise((resolve, reject) => {
        ...
        // If the process dies before the done line, reject instead of waiting
        // forever — the done line is never coming.
        this.mcServer.on('close', (code) => {
          self.mcServer = null
          const tail = recentLines.join(' | ')
          reject(new Error(`server exited before becoming ready (code ${code})${tail ? `: ${tail}` : ''}`))
        })
