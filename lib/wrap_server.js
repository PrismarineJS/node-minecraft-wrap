...
class WrapServer extends EventEmitter {
  ...
  deleteServerData () {
    return new Promise(resolve => { rimraf(this.MC_SERVER_PATH, resolve) })
  }
  ...
  async startServer (propOverrides) {
    ...
    await mkdirp(this.MC_SERVER_PATH)
    this.OPTIONS.noOverride
      ? Promise.resolve()
      : Promise.all([
    ...
    try {
      await fs.promises.access(this.MC_SERVER_JAR, fs.constants.W_OK | fs.constants.R_OK)
    } catch (err) {
      throw new Error('The file ' + this.MC_SERVER_JAR + " doesn't exist.")
    }
