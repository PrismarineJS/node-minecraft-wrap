...
class WrapServer extends EventEmitter {
  constructor (MC_SERVER_JAR, MC_SERVER_PATH, OPTIONS) {
    ...
    this.OPTIONS = {
      ...
      disableProgressStatus: (OPTIONS && OPTIONS.disableProgressStatus) ? OPTIONS.disableProgressStatus : false
    }
  }
    ...
    mkdirp(this.MC_SERVER_PATH)
      ...
      .then(() => new Promise((resolve, reject) => {
        ...
        this.mcServer.on('line', (line) => {
          if (!self.OPTIONS.disableProgressStatus) process.stderr.write('.')
          this.emit('line', line)
        })
