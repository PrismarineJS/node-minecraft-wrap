...
class LauncherDownload {
  ...
  getVersionsList () {
    ...
    // Route through downloadFile so the manifest gets the same timeout and
    // retry handling as every other download (a bare fetch can hang forever).
    const manifestPath = `${this.mcPath}/versions/version_manifest.json`
    return downloadFile('https://launchermeta.mojang.com/mc/game/version_manifest.json', manifestPath, null, null)
      .then(() => fs.readFile(manifestPath, 'utf8'))
      .then((data) => {
        this.versionsList = JSON.parse(data)
...
async function downloadWithRetry (url, path, size, sha1) {
  for (let attempt = 1; ; attempt++) {
    try {
      ...
      await fs.unlink(path).catch(() => {}) // drop partial files
...
function downloadOnce (url, path) {
  return queue.add(() => new Promise((resolve, reject) => {
    // A stalled connection must eventually reject, or callers waiting on the
    ...
    fetch(url, { signal: controller.signal }).then(res => {
      ...
      res.body.on('error', err => { clearTimeout(timeoutHandle); reject(err) })
      fileStream.on('error', err => { clearTimeout(timeoutHandle); reject(err) })
      fileStream.on('finish', () => { clearTimeout(timeoutHandle); resolve() })
    }).catch(err => {
