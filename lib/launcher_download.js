...
class LauncherDownload
{
  ...
  _parseLibRules(rules) {
    ...
    if(rules) {
      ...
      rules.forEach(({action,os}) => {
        if(action==="allow" && ((os && os.name===this.os) || !os))
          skip=false;

        if(action==="disallow" && ((os && os.name===this.os) || !os))
          skip=true;
      })
    }
    return skip;
  ...
  getLibraries(version) {
    return this.getVersionInfos(version)
      .then(versionInfo => {
        const libsIndexed=versionInfo["libraries"].reduce((acc,lib) => {acc[lib['name']]=lib;return acc;},{});
        const libUnique=Object.keys(libsIndexed).map(k => libsIndexed[k]);

        return Promise.all(flatmap(libUnique, lib => {

          if(this._parseLibRules(lib["rules"]))
            return [];
          ...
          if(lib['downloads']['classifiers'] && lib['downloads']['classifiers']["natives-"+this.os])
            results.push(getLib(lib['downloads']['classifiers']["natives-"+this.os]));
          return results;
        }));
      });
  }
